...
class Cnpj extends AbstractGenerator
{
    ...
    public function isRegisteredDatabase(string $cnpj): bool
    {
        $client = new Client(config: ["base_uri" => "http://localhost"]);

        $response = $client->request(
            method: HttpRequest::GET,
            uri: sprintf("v1/persons/1%s", $cnpj),
            options: [
                RequestOptions::HTTP_ERRORS => false
            ]
        );
        $response = json_decode(json: $response->getBody()->getContents(), associative: true);
